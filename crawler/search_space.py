...
class SearchSpaceGenerator:
    ...
    def generate_all_partitions(self) -> Iterator[SearchPartition]:
        ...
        for lang, stars in itertools.product(self.languages, self.star_ranges):
            ...
            query = (
                f'is:public language:"{lang}" stars:{stars} fork:false archived:false'
            )
        ...
        for time_range, stars in itertools.product(
            self.time_ranges, self.star_ranges[:50]
        ):
    ...
    def estimate_total_partitions(self) -> int:
        ...
        total += (
            min(20, len(self.size_ranges))
            * min(20, len(self.languages))
            * min(20, len(self.star_ranges))
        )
