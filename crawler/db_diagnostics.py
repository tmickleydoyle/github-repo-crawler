...
class DatabaseDiagnostics:
        ...
        try:
            ...
            return {
                ...
                "data_integrity": {
                    ...
                    "consistency_rate": round((discovered_count - missing_repos) / discovered_count * 100, 1) if discovered_count > 0 else 100
                },
    ...
    async def test_storage_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                await conn.execute("""
                    INSERT INTO repo (id, name, owner, url, name_with_owner, alphabet_partition)
                    VALUES ($1, $2, $3, $4, $5, $6)
                    ON CONFLICT (id) DO NOTHING
                """, test_repo_id, "test-repo", "test-user",
                    "https://github.com/test-user/test-repo", "test-user/test-repo", "t")
                ...
                await conn.execute("""
                    INSERT INTO discovered_repositories (repo_id, matrix_index, crawl_run_id)
                    VALUES ($1, $2, $3)
                    ON CONFLICT (repo_id) DO NOTHING
                """, test_repo_id, 99, "test-run")
                test_results["discovered_insert"] = True

                # Cleanup test data
                await conn.execute("DELETE FROM repo_stats WHERE repo_id = $1", test_repo_id)
                await conn.execute("DELETE FROM discovered_repositories WHERE repo_id = $1", test_repo_id)
    ...
    async def generate_comprehensive_report(self) -> str:
        ...
        schema = await self.check_schema_health()
        ...
        report.append(f"Usage: {storage['storage']['size_mb']} MB ({storage['storage']['usage_percent']}% of free tier)")
        ...
        report.append(f"\n🔍 DATA INTEGRITY")
        report.append("-" * 40)
        report.append(f"Consistency Rate: {storage['data_integrity']['consistency_rate']}%")
        if storage['data_integrity']['missing_repos'] > 0:
            report.append(f"⚠️  Missing repos in main table: {storage['data_integrity']['missing_repos']:,}")
        else:
        ...
        if performance['recent_runs']:
            ...
            for run in performance['recent_runs'][:5]:
                ...
                report.append(f"  • {run['crawl_run_id']}: {run['repos_discovered']:,} repos, {run['jobs_used']} jobs")
        ...
        if functionality.get('error'):
            ...
            all_passed = all(functionality[key] for key in ['repo_insert', 'repo_stats_insert', 'discovered_insert', 'cleanup'])
            report.append("✅ All storage operations working correctly" if all_passed else "⚠️  Some storage issues detected")

        report.append("\n" + "=" * 80)
        return "\n".join(report)


async def run_diagnostics_cli():
